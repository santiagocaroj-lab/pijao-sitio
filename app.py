...
# 1. PÁGINA DE BIENVENIDA (Sin indentación para evitar Markdown bug)
...
<video width="100%" autoplay muted loop playsinline style="border-radius: 4px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); background: #FAF8F5;">
...
# 3. VIDEO CINEMATOGRÁFICO PRINCIPAL
st.markdown("""
...
<div style="max-width: 1100px; margin: 0 auto; padding: 0 20px;">
<video width="100%" controls preload="auto" style="border-radius: 4px; box-shadow: 0 20px 40px rgba(0,0,0,0.4); background: #FAF8F5;">
<source src="{video_intro_b64}" type="video/mp4">
Tu navegador no soporta video HTML5.
</video>
</div>
...
hist_imgs = [photos_b64.get(k) for k in ["F2", "F5", "F6", "F16", "F17"] if k in photos_b64]
if hist_imgs:
    st.markdown("""
...
if current_item_key.startswith("F"):
    ...
    if img_b64:
        st.markdown(f'<div style="margin: 20px 0;"><img src="{img_b64}" style="width: 100%; max-height: 550px; object-fit: contain; border-radius: 2px;"></div>', unsafe_allow_html=True)
    else:
        st.info(f"Recurso {current_item_key} no disponible en la carpeta assets.")
elif current_item_key == "VPINICIO" and vpinicio_b64:
    st.markdown(f'<div style="margin: 20px 0;"><video width="100%" controls autoplay muted loop playsinline style="border-radius: 2px;"><source src="{vpinicio_b64}" type="video/mp4"></video></div>', unsafe_allow_html=True)
elif current_item_key == "VIDEO" and video_intro_b64:
    st.markdown(f'<div style="margin: 20px 0;"><video width="100%" controls preload="auto" style="border-radius: 2px;"><source src="{video_intro_b64}" type="video/mp4"></video></div>', unsafe_allow_html=True)
